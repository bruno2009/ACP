...
Let a non-null vector $v$ (i.e. $\vec{v} \neq 0$), $v \in V$ (vector space) is said a eigenvectors of $T$ if there exist a real number $\lambda$ which $T(v) = \lambda v$. The $\lambda$ scalar is denominated a eigenvalues of $T$ that is associate a $v$. It can be concluded that $v$ and $T$ have the same support line (and thus the same direction.):
</p>
</div>
...
in which $I$ is identity matrix. So there are two solutions.

...
For a random sample vector $x_{l} = (x_{l1}, \dots, x_{lp})^{\top}$ with $l = 1, \dots, n$ and sample mean vector
...
Now let $Y_{1 \times p} = (y_{1}, \dots, y_{p})^{\top} \in \mathbb{R}_{\small\setminus0}^{p}$ be any column vector. Thus

<div class="equation">
$$
Y^{\top}SY = Y^{\top}\bigg( \cfrac{1}{n - 1} \sum_{l = 1}^{n} (x_{l} - \overline{x})(x_{l} - \overline{x})^{\top} \bigg)Y
$$
</div>

how $Y$ does not dependent on $l$, we can move into the summation

<div class="equation">
$$
Y^{\top}SY = \bigg( \cfrac{1}{n - 1} \sum_{l = 1}^{n} \underset{1 \times p}{Y^{\top}}\underset{p \times 1}{(x_{l} - \overline{x})}\underset{1 \times p}{(x_{l} - \overline{x})^{\top}}\underset{p \times 1}{Y} \bigg)
$$
</div>
<div class="equation">
$$
Y^{\top}SY = \cfrac{1}{n - 1} \sum_{l = 1}^{n} \Big( (x_{l} - \overline{x})^{\top}Y \Big)^{2} \geq 0 
$$
</div>

Thereby, the covariance matrix will always be a positive semidefinite matrix. Notice that $Y^{\top}SY$ will only be zero whether $(x_{l} - \overline{x})^{\top}Y = 0$ for all $l = 1, \dots, n$.

Another point that you need to take in mind is $\text{det}(S) = 0$, which means $Y^{\top}SY = 0$; in this situation, $S$ is a singular matrix, i.e., there is no inverse. In other words, $S$ is ill-conditioned, and in my opinion, you need to begin trying if there is a collinearity problem. Or we can use [Moore-Penrose generalized inverse](https://en.wikipedia.org/wiki/Moore%E2%80%93Penrose_inverse) to have a pseudoinverse matrix. 

# PRINCIPAL COMPONENT ANALYSIS

The PCA is a technique that will find a direction of form in which the projection of data has a greater variance. This direction is, by definition, called the first principal direction.

Consider the following optimization problem $e_{i} \in \mathbb{R}^{p}$

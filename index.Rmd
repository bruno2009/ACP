...
<li> It is always possible to obtain a basis $P$ which $P^{\top} = [e_{1}, \dots, e_{p}]$ from orthonormal eigenvectors. **Then:**

<div class="equation">
$$ \begin{matrix}
 \cfrac{\vec{v}_{1}}{||\vec{v}_{1}||} = (e_{11}, e_{12}, \dots, e_{1p})^{\top} = e_{1}^{\top} \\
  \vdots\\
 \cfrac{\vec{v}_{p}}{||\vec{v}_{p}||} = (e_{p1}, e_{p2}, \dots, e_{pp})^{\top} = e_{p}^{\top} 
\end{matrix}$$
</div>

with $e_{i}^{\top} e_{j} = 0$ and $e_{i}^{\top} e_{i} = 1$, $i \neq j$ and $i, j = 1, \dots, p$;
